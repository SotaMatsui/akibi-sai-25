...
function AccessSection({ className, id }: { className?: string; id?: string }) {
  ...
  return (
    <div className="w-full" ref={scope}>
      <div
        className={`hidden md:flex gsap-horizontal-trigger min-h-screen justify-center min-w-0 overflow-x-hidden w-full ${className}`}
        id={id}
      >
      ...
      <div className="min-h-screen flex flex-col md:hidden items-center justify-center gap-4 p-4 pb-32">
        <div
          className={`text-end gap-2 py-8 text-7xl font-medium font-display ${className}`}
        >
          アク
          <br />
          セス
        ...
        <iframe
          ...
          loading="lazy"
          className="w-full aspect-[4/3] border-0 rounded-3xl z-10"
        />
        <div className="flex flex-col items-start justify-center">
          <h2 className="text-center font-serif py-4 text-4xl">
            秋田公立美術大学
          </h2>
          <div className="text-left font-semibold text-sm space-y-2">
            <p>〒010-1632 秋田県秋田市新屋大川町12-3</p>
            <div>
              <p>JR「秋田駅」から羽越本線「新屋駅」下車　新屋駅から徒歩15分</p>
              <p>
                JR「秋田駅」から秋田中央交通バス
                新屋線「美術大学前」下車　徒歩1分
              </p>
